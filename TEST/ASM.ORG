...
niteia  proc    far
        ...
        mov     chksum,00h
        mov     ah,35h          ;get com2 vector
        mov     al,0bh
        int     21h
        mov     intcseg,es
        mov     intcoff,bx
        mov     al,0dh          ;get irq5 vector
        mov     int5seg,es
        mov     int5off,bx
        mov     dx,cs
        mov     ds,dx
        ...
        mov     ax,250dh        ;irq5
        int     21h             ;set interrupt vector
        in      al,21h          ;get the current PIC mask
        and     al,0d7h         ;enable INT3 and int5
        out     21h,al          ;write new mask for com2
        mov     dx,02f9h
